...
### 2. Dual-Directory Mirroring Rule (`public/` and Root)
- The GitHub Pages deployment workflow (`.github/workflows/deploy.yml`) deploys the `./public` directory.
- Root files (`index.html`, `js/`, `css/`, etc.) exist alongside `public/`.
- When making modifications:
  1. Make edits in `public/` (the canonical source).
  2. Mirror the changes to root (or keep them synchronized) so root files are never an outdated copy.
  3. Never copy older root files over newer `public/` files.

### 3. Verification & Push Protocol
Before pushing any changes to GitHub:
1. **Check Git Status & Diffs**: Run `git status` and `git diff` to inspect every modified line. Ensure no existing features, permissions, or configurations were accidentally removed or reverted.
...
3. **Syntax / Code Check**: Validate all modified scripts (`node --check ...`) to prevent runtime syntax errors.
...
---
**Core Rule**:  
`Latest Version → Add/Edit Specific Feature → Test → Commit → Push`  
